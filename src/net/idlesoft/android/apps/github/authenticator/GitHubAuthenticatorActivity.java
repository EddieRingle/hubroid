...
public class GitHubAuthenticatorActivity extends BaseActivity
{
	...
	@InjectView(id.et_auth_field_login)
	private AutoCompleteTextView mLoginText;

	@InjectView(id.et_auth_field_password)
	private EditText mPasswordText;

	@InjectView(id.btn_auth_sign_in)
	...
	/**
	 * If set we are just checking that the user knows their credentials; this doesn't cause the user's mPassword to be
	...
	public void onCreate(Bundle bundle) {
		super.onCreate(bundle, layout.login_activity);
	...
	protected Dialog onCreateDialog(int id) {
		...
		dialog.setMessage(getText(string.auth_message_signing_in));
	...
	/**
	 * Handles onClick event on the Submit button. Sends username/mPassword to the server for authentication.
	 * <p/>
		...
		mAuthenticationTask = new RoboAsyncTask<Boolean>(this) {
			...
			protected void onException(Exception e) throws RuntimeException {
				...
				if ("Received authentication challenge is null".equals(cause.getMessage()))
					message = getResources().getString(string.auth_message_bad_credentials);
				else
	 ...
	 * the AccountAuthenticatorResult which is sent back to the caller. Also sets the mAuthToken in AccountManager for
	 * this account.
	 */
	...
	public void onAuthenticationResult(boolean result) {
		if (result)
			...
			if (mRequestNewAccount)
				ToastUtil.toastOnUiThread(GitHubAuthenticatorActivity.this, string.auth_message_auth_failed_new_account);
			else
				ToastUtil.toastOnUiThread(GitHubAuthenticatorActivity.this, string.auth_message_auth_failed);
		}
	}
