    ...
    protected void buildUI() {
        if (mUser != null) {
            ...
            try {
                ...
                listItems.put(buildListItem("Public Activity", "View " + mTarget
                        + "'s public activity"));
                listItems.put(buildListItem("Repositories",
                        Integer.toString(mUser.getPublicRepos() + mUser.getTotalPrivateRepos())));
                if (IsNotNullNorEmpty(mUser.getType())) {
                    if (mUser.getType().equalsIgnoreCase("Organization")) {
                        listItems.put(buildListItem("Members & Teams",
                                "Find out who makes this organization tick"));
                    }
                }
                listItems.put(buildListItem(
                        "Followers / Following",
                        Integer.toString(mUser.getFollowers()) + " / "
                                + Integer.toString(mUser.getFollowing())));
                listItems.put(buildListItem("Gists",
                        Integer.toString(mUser.getPublicGists() + mUser.getPrivateGists())));
                if (IsNotNullNorEmpty(mUser.getCreatedAt().toString())) {
                    listItems.put(buildListItem("Join Date", mUser.getCreatedAt().toString()));
                }
