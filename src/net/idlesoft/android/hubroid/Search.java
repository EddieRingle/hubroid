...
public class Search extends Activity {
	...
	private static class SearchTask extends AsyncTask<String, Void, Void> {
	    ...
	    protected void onPreExecute() {
	        if (mActivity.m_type.equals(REPO_TYPE)) {
	            ((TextView)mActivity.mLoadingItem.findViewById(R.id.tv_loadingListItem_loadingText)).setText("Searching for Repositories...");
	            mActivity.mListView.addHeaderView(mActivity.mLoadingItem);
	        } else if (mActivity.m_type.equals(USER_TYPE)) {
                ((TextView)mActivity.mLoadingItem.findViewById(R.id.tv_loadingListItem_loadingText)).setText("Searching for Users...");
                mActivity.mListView.addHeaderView(mActivity.mLoadingItem);
	        }
	    ...
	    protected void onProgressUpdate(Void... progress) {
	        if (mActivity.m_type.equals(REPO_TYPE)) {
	            Toast.makeText(mActivity, "Error gathering repository data, please try again.", Toast.LENGTH_SHORT).show();
	        } else if (mActivity.m_type.equals(USER_TYPE)) {
	            Toast.makeText(mActivity, "Error gathering user data, please try again.", Toast.LENGTH_SHORT).show();
	        }
	    }
        ...
        protected Void doInBackground(String... params) {
            if (mActivity.m_type.equals(REPO_TYPE)) {
                try {
                    JSONObject response = new JSONObject(mActivity._gapi.repo.search(params[0]).resp);
                    mActivity.m_repositoriesData = response.getJSONArray(REPO_TYPE);
                    mActivity.m_repositories_adapter = new RepositoriesListAdapter(mActivity, mActivity.m_repositoriesData);
                ...
                try {
                    JSONObject response = new JSONObject(mActivity._gapi.user.search(params[0]).resp);
                    mActivity.m_usersData = response.getJSONArray(USER_TYPE);
                    mActivity.m_users_adapter = new SearchUsersListAdapter(mActivity, mActivity.m_usersData);
                } catch (JSONException e) {
                    publishProgress((Void)null);
                    e.printStackTrace();
                }
	...
	public void search()
	{
		...
		if (mSearchTask.getStatus() == AsyncTask.Status.PENDING) {
		    mSearchTask.execute(((EditText)findViewById(R.id.et_search_search_box)).getText().toString());
		}
	}
	...
	public void onCreate(Bundle icicle) {
		...
		((TextView)mLoadingItem.findViewById(R.id.tv_loadingListItem_loadingText)).setText("Searching Repositories, Please Wait...");
		mReposBtn.setEnabled(false);
