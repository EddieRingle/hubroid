	...
	public View getView(int index, View convertView, ViewGroup parent) {
		...
		try {
			...
			if (year > 0) {
				...
				holder.meta.setText("Posted " + year + end + " by " + m_data.getJSONObject(index).getString("user"));
			}
			if (day > 0) {
				if (day == 1) {
					end = " day ago";
				...
				holder.meta.setText("Posted " + day + end + " by " + m_data.getJSONObject(index).getString("user"));
			} else if (hour > 0) {
				if (hour == 1) {
					end = " hour ago";
				} else {
				...
				holder.meta.setText("Posted " + hour + end + " by " + m_data.getJSONObject(index).getString("user"));
			} else if (min > 0) {
				if (min == 1) {
					end = " minute ago";
				} else {
				...
				holder.meta.setText("Posted " + min + end + " by " + m_data.getJSONObject(index).getString("user"));
			} else {
				if (sec == 1) {
					end = " second ago";
				} else {
