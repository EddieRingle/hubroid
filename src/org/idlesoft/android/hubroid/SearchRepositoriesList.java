...
public class SearchRepositoriesList extends ListActivity {
		...
		try {
			...
			if (m_jsonData == null) {
				runOnUiThread(new Runnable() {
					public void run() {
						Toast.makeText(SearchRepositoriesList.this, "Error gathering repository data, please try again.", Toast.LENGTH_SHORT).show();
					}
				});
			} else {
	...
	private Runnable threadProc_itemClick = new Runnable() {
		public void run() {
			try {
	        	m_intent = new Intent(SearchRepositoriesList.this, RepositoryInfo.class);
			...
			runOnUiThread(new Runnable() {
				public void run() {
					SearchRepositoriesList.this.startActivity(m_intent);
				}
			});
		}
	};
	...
	private OnClickListener m_btnSearchListener = new OnClickListener() {
		public void onClick(View v) {
			...
			if (m_searchBox.getText().toString() != "") {
				m_progressDialog = ProgressDialog.show(SearchRepositoriesList.this, "Please wait...", "Searching Repositories...", true);
